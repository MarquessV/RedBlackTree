...
class node {
  public:
    ...
    node(const T&);
    node(node<T>*, const T&);                              // Constructor for node with known parent.
...
template<class T>
node<T>::node(const T& d) : parent(nullptr), left(nullptr), right(nullptr), isRed(false), data(d) {}

template<class T>
node<T>::node(node<T>* p, const T& d) : parent(p), left(nullptr), right(nullptr), isRed(true), data(d) {}

template<class T>
node<T>::node(node<T>* p, node<T>* l, node<T>* r, const T& d) : parent(p), left(l), right(r), isRed(true), data(d) {}

template<class T>
node<T>::node(node<T>* p, node<T>* l, node<T>* r, bool red, const T& d) : parent(p), left(l), right(r), isRed(true), data(d) {}

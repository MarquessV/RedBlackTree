...
class node {
  ...
  public:
    ...
    node(node<T>*, node<T>*, const T&);         // Constructor for a node with children and type
    node(node<T>*, node<T>*, bool, const T&);   // Constructor for a node with known color.
...
template<class T>
node<T>::node(node<T>* l, node<T>* r, const T& d) : left(l), right(r), data(d) {}

template<class T>
node<T>::node(node<T>* l, node<T>* r, bool red, const T& d) : left(l), right(r), isRed(red), data(d) {}
